...
def train_epoch(epoch, model, train_loader, loss_criterion, optimizer, device, cfg=None):
...
@torch.no_grad()
def test_epoch(epoch, model, dataloader, loss_criterion, device, cfg=None, vis_dir=None):

    batch_loss    = AverageMeter()
    model.eval()
    
    t_start = time.time()
    for batch_idx, (imgs, hms) in enumerate(tqdm(dataloader, desc='[(TEST) Epoch {}]'.format(epoch))):
        imgs = imgs.to(device)
        for scale, hm in hms.items():
            hms[scale] = hm.to(device)
        preds  = model(imgs)
