...
# make precompute/ importable so we can reach helpers/frame_source.py
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))
from helpers.frame_source import read_frame_at  # <-- use the helper here
...
VIS          = bool(pose_cfg.get("visualize", True))
...
with open(out_path, "w", encoding="utf-8") as jf:
    for ev_id, t in events:
        ...
        for i in range(start, t+1):
            img = read_frame_at(video_path, i)   # <-- fetch by absolute frame index
