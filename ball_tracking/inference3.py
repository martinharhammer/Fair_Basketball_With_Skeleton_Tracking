...
from postprocessor2 import WASBPostprocessor   # assumes you copied their postprocessor.py

def main():
    # --------------- CONFIGURATION ---------------
    PRECOMPUTE_CONFIG = '/home/ubuntu/precompute/config.json'
    CONFIG_PATH   = '/home/ubuntu/ball_tracking/config_hrnet.yaml'
    WEIGHTS_PATH  = '/home/ubuntu/ball_tracking/training/finetune_outputs/best_finetuned.pth.tar'

    with open(PRECOMPUTE_CONFIG, "r", encoding="utf-8") as f:
        C = json.load(f)

    FRAMES_FOLDER = C["frames_dir"]
    ...
    #jsonl_path = os.path.join(run_dir, "detections.jsonl") 
    jsonl_path = OUT_PATH
    ...
    post = WASBPostprocessor(
        ...
        distance_threshold=5,       # tweak if needed
        history_size=3
    )
    # --------------- PREPROCESSOR ---------------
    ...
    with open(jsonl_path, "w") as jf:
        for i in tqdm(range(len(frames) - N_FRAMES + 1), desc='Inference'):
            ...
            if xy is not None:
                ...
                xy_scaled = [float(xy[0] * scale_x), float(xy[1] * scale_y)]
            else:
                xy_scaled = None
            ...
            print(f'[{i+1}] saved → {vis_fname} in {run_dir}')
    ...
    # -------- CONFIG --------
    overlay_folder = OUTPUT_FOLDER  # path to your overlay images
    ...
    #'''
    print(f"[DEBUG]: Done! Wrote to {jsonl_path}")
